...
ggplot(track, aes(x = Seconds)) +  # Map the converted seconds to the x-axis
  ...
  labs(x = "Time (s:ms)", y = "Elevation (in m)", color = NULL) +  # Add x-axis label as "Time"
...
```{r accel-accel, echo=FALSE, fig.cap="Vertical Acceleration as measured by the pressure sensor", warning=FALSE}
...
ggplot(track, aes(x = Seconds)) + 
  geom_line(aes(y = Total.Distance.Lowess, color = "Distance LOWESS"), size = 1, linetype = "solid") +
...
ggplot(track, aes(x = Seconds)) + 
  ...
  scale_x_continuous(breaks = pretty(track$Seconds, n = 10), labels = seconds_milliseconds) +
  labs(x = "Time (s:ms)", y = "Speed (inm/s)", color = NULL) +  
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        legend.position = c(0.85, 0.1)) +
...
## Failure Analysis - Parachute, SD-Card
